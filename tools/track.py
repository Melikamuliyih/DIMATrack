...
def make_parser():
    ...
    parser.add_argument("--fast-reid-config", dest="fast_reid_config", default=r"/root/userfolder/BoT-SORT/fast_reid/configs/MOT17/sbs_S50.yml", type=str, help="reid config file path")
    parser.add_argument("--fast-reid-weights", dest="fast_reid_weights", default=r"/root/userfolder/BoT-SORT/logs/MOT17/sbs_S50/market_bot_S50.pth", type=str, help="reid config file path")
...
def image_track(predictor, vis_folder, args):
    ...
    for frame_id, img_path in enumerate(files, 1):
        ...
        if outputs[0] is not None:
            ...
            detections[:, :4] /= scale
            # print("det size", detections.shape)
...
def main(exp, args):
    ...
    if args.ckpt is None:
        ckpt_file = osp.join(output_dir, "mot17_sbs_S50.pth")
    else:
        ckpt_file = args.ckpt
    ...
    # print("model load: ", model.load_state_dict(ckpt["model"]))
    ...
    if args.benchmark == 'MOT20':
        results_folder = 'YOLOX_outputs/yolox_x_mix_mot20_ch/track_results'
    else:
    ...
    if args.benchmark == 'MOT20':
        gtfiles = glob.glob(
        os.path.join('/root/userfolder/BoT-SORT/tools/datasets/MOT20/train', '*/gt/gt{}.txt'.format(gt_type)))
    else:
        gtfiles = glob.glob(
        os.path.join('/root/userfolder/BoT-SORT/tools/datasets/MOT17/train', '*/gt/gt{}.txt'.format(gt_type)))
        ...
        for i in seqs:
            ...
            if args.default_parameters:
                ...
                if MOT == 20:  # MOT20
                    ...
                    args.ckpt = r'/root/userfolder/BoT-SORT/logs/MOT17/sbs_S50/bytetrack_x_mot20.tar'
                    args.match_thresh = 0.7
                else:  # MOT17
                    if ablation:
                        args.exp_file = r'./yolox/exps/example/mot/yolox_x_ablation.py'
                        args.ckpt = r'/root/userfolder/BoT-SORT/logs/MOT17/sbs_S50/jdecmc_best_mot17.pth.tar'
                    else:
